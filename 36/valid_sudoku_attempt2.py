...
def isValidSudoku2(board: List[List[str]]) -> bool:
    ...
    #            if (board[i].count(board[i][j]) > 1 or
    #                [row[j] for row in board].count(board[i][j]) > 1 or
    #                [row[j] for row in board[i//3*3:i//3*3+3]
    #                                        for row in row[j//3*3:j//3*3+3]].count(board[i][j]) > 1):
    #                return False
    #return True
